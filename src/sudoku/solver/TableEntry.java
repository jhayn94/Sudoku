...
//    private static final long RAW_ENTRY      = 0x1fffffffffffffffL;
	...
	void addEntry(int cellIndex, int cand, int penalty, boolean set) {
		this.addEntry(cellIndex, -1, -1, Chain.NORMAL_NODE, cand, set, 0, 0, 0, 0, 0, penalty);
	}
	...
	void addEntry(int cellIndex, int cand, boolean set) {
		this.addEntry(cellIndex, -1, -1, Chain.NORMAL_NODE, cand, set, 0, 0, 0, 0, 0, 0);
	}
	...
	void addEntry(int cellIndex, int cand, boolean set, int reverseIndex) {
		this.addEntry(cellIndex, -1, -1, Chain.NORMAL_NODE, cand, set, reverseIndex, 0, 0, 0, 0, 0);
	}
	...
	void addEntry(int cellIndex, int cand, boolean set, int ri1, int ri2, int ri3, int ri4, int ri5) {
		this.addEntry(cellIndex, -1, -1, Chain.NORMAL_NODE, cand, set, ri1, ri2, ri3, ri4, ri5, 0);
	}
	...
	void addEntry(int cellIndex1, int alsIndex, int nodeType, int cand, boolean set, int penalty) {
		this.addEntry(cellIndex1, Chain.getSLowerAlsIndex(alsIndex), Chain.getSHigherAlsIndex(alsIndex), nodeType, cand,
				set, 0, 0, 0, 0, 0, penalty);
	}
	...
	void addEntry(int cellIndex1, int cellIndex2, int cellIndex3, int nodeType, int cand, boolean set, int ri1, int ri2,
		...
		final int entry = Chain.makeSEntry(cellIndex1, cellIndex2, cellIndex3, cand, set, nodeType);
	...
	int getEntryIndex(int cellIndex, boolean set, int cand) {
		/// *K*/ returns null???
		final Integer ret = this.indices.get(Chain.makeSEntry(cellIndex, cand, set));
		if (ret == null) {
...
//        return indices.get(Chain.makeSEntry(cellIndex, cand, set));
	...
	int getEntryIndex(int entry) {
		final Integer tmp = this.indices.get(entry);
		if (tmp == null) {
...
//                (rIndices[1] << 12) + rIndices[0];
	...
	public void setDistance(int index, int distance) {
		...
		final long tmp = distance & 0x1ff;
		this.retIndices[index] &= 0xE00FFFFFFFFFFFFFL;
		this.retIndices[index] |= (tmp << 52);
	}
	...
	public int getDistance(int index) {
		return getSRetIndex(this.retIndices[index], 5) & 0x1ff;
	}
	...
	public boolean isExpanded(int index) {
		return (this.retIndices[index] & EXPANDED) != 0;
	}
	...
	public void setExpanded(int index) {
		this.retIndices[index] |= EXPANDED;
	}
	...
	public boolean isOnTable(int index) {
		return (this.retIndices[index] & ON_TABLE) != 0;
	}
	...
	public void setOnTable(int index) {
		this.retIndices[index] |= ON_TABLE;
	}
	...
	public boolean isExtendedTable(int index) {
		return (this.retIndices[index] & EXTENDED_TABLE) != 0;
	}
	...
	public void setExtendedTable(int index) {
		this.retIndices[index] |= EXTENDED_TABLE;
	}
	...
	public void setExtendedTable() {
		this.retIndices[this.index - 1] |= EXTENDED_TABLE;
	}
