...
public class AlsSolver extends AbstractSolver {
	...
	protected List<SolutionStep> getAllAlses(boolean doXz, boolean doXy, boolean doChain) {
		...
		final List<SolutionStep> resultSteps = new ArrayList<SolutionStep>();
...
//            }
			...
			this.firstRC = null;
			if (DEBUG) {
				System.out.println("============== Start search: " + i + " " + this.startAls);
			}
			this.getAlsXYChainRecursive(i, null);
			if (DEBUG) {
				System.out.println("               End search: " + this.alses.get(i));
			}
		}
		if (DEBUG) {
			System.out.println(this.steps.size() + " (maxRecDepth: " + this.maxRecDepth + ")");
		}
	}
...
//                showActAlsChain(recDepth);
	...
	private void showActAlsChain(int recDepth) {
		if (DEBUG) {
			...
			System.out.println(this.globalStep.toString(2));
		}
	}
				...
				if (cand < this.maxDBCand) {
					...
					for (int j = 0; j < cands.length; j++) {
						...
						for (int k = 0; k < this.aktDBAls.length; k++) {
							...
							if (first) {
								this.dbIndicesPerCandidate
										.set(this.alses.get(this.aktDBAls[k]).indicesPerCandidat[checkCand]);
								first = false;
							} else {
								this.dbIndicesPerCandidate
										.or(this.alses.get(this.aktDBAls[k]).indicesPerCandidat[checkCand]);
							}
						}
		...
		if (withChain) {
			...
			for (int i1 = 0; i1 < als1.indicesPerCandidat[cand].size(); i1++) {
				for (int i2 = 0; i2 < als2.indicesPerCandidat[cand].size(); i2++) {
					final int index1 = als1.indicesPerCandidat[cand].get(i1);
					final int index2 = als2.indicesPerCandidat[cand].get(i2);
...
//                System.out.println("collectAllRestrictedCommons(): " + ticks + "ms; restrictedCommon size: " + restrictedCommons.size());
	...
	private void collectAllAlses() {
		...
		if (DEBUG) {
			System.out.println("collectAllAlses(): " + (ticks / 1000000.0) + "ms; alses size: " + this.alses.size());
		}
	}
		...
		for (int i = 0; i < this.alses.size(); i++) {
			...
			for (int j = 1; j <= 9; j++) {
				...
				for (int k = 0; k < act.buddiesPerCandidat[j].size(); k++) {
					final int index = act.buddiesPerCandidat[j].get(k);
